...
const MACHINE_SECRET_FILE: &str = "machine_secret.key";
...
impl SecureStorage {
    pub fn new(app_data_dir: PathBuf) -> Self {
        let machine_id = Self::resolve_machine_id(&app_data_dir);
    ...
    /// Resolve machine_id: use machine_secret.key if it exists, otherwise handle migration.
    /// - If machine_secret.key exists, use its hex-encoded contents.
    /// - If it doesn't exist but encrypted credentials do (legacy), try whoami first,
    ///   then create the key file and re-encrypt with the new key.
    /// - If neither exists (fresh install), generate a new random key file.
    fn resolve_machine_id(app_data_dir: &PathBuf) -> String {
        let secret_path = app_data_dir.join(MACHINE_SECRET_FILE);
        let storage_path = app_data_dir.join(STORAGE_FILE);

        // If secret file already exists, use it
        if let Ok(bytes) = fs::read(&secret_path) {
            if bytes.len() == MACHINE_SECRET_LEN {
                return bytes.iter().map(|b| format!("{:02x}", b)).collect::<String>();
            }
        }

        // Check if legacy credentials exist (need migration)
        let has_legacy_credentials = storage_path.exists();

        if has_legacy_credentials {
            // Legacy migration: use whoami as machine_id for now.
            // After successful load_credentials(), we'll re-encrypt with new key.
            let legacy_id = whoami::devicename();

            // Generate and save the new secret for future use
            let new_secret = Self::generate_and_save_secret(app_data_dir);

            // Try to migrate: load with legacy key, save with new key
            let legacy_storage = SecureStorage {
                storage_path: storage_path.clone(),
                cache: Mutex::new(HashMap::new()),
                encryption_key: Mutex::new(None),
                salt: Mutex::new(None),
                machine_id: legacy_id,
            };

            if let Ok(credentials) = legacy_storage.load_credentials() {
                if !credentials.is_empty() {
                    // Re-encrypt with new machine_id
                    let new_storage = SecureStorage {
                        storage_path,
                        cache: Mutex::new(HashMap::new()),
                        encryption_key: Mutex::new(None),
                        salt: Mutex::new(None),
                        machine_id: new_secret.clone(),
                    };
                    if let Ok(()) = new_storage.save_credentials(&credentials) {
                        info!("[SecureStorage] Migrated credentials to machine_secret.key");
                    }
                }
            }
            ...
            // Fresh install
            Self::generate_and_save_secret(app_data_dir)
        }
    }

    fn generate_and_save_secret(app_data_dir: &PathBuf) -> String {
        use aes_gcm::aead::rand_core::RngCore;
        let mut secret = [0u8; MACHINE_SECRET_LEN];
        OsRng.fill_bytes(&mut secret);
        if let Some(parent) = app_data_dir.join(MACHINE_SECRET_FILE).parent() {
            let _ = fs::create_dir_all(parent);
        }
        let _ = fs::write(app_data_dir.join(MACHINE_SECRET_FILE), &secret);
    ...
    pub(crate) fn load_credentials(&self) -> Result<HashMap<String, String>, SecureStorageError> {
        ...
        if !encrypted_data.is_empty() && encrypted_data[0] != FORMAT_VERSION_V1 {
            if let Ok(()) = self.save_credentials(&credentials) {
                info!("[SecureStorage] Migrated credentials to Argon2id format");
            }
        }
    ...
    pub(crate) fn save_credentials(&self, credentials: &HashMap<String, String>) -> Result<(), SecureStorageError> {
...
pub fn secure_storage_set_provider_keys(
    ...
    let mut credentials = storage.load_credentials()?;
    for (provider_uuid, api_key) in &provider_keys {
        let key = format!("provider_{}", provider_uuid);
        credentials.insert(key.clone(), api_key.clone());
        if let Ok(mut cache) = storage.cache.lock() {
            cache.insert(key, api_key.clone());
        }
    }
...
pub fn secure_storage_get_provider_keys(
    ...
    let credentials = storage.load_credentials()?;

    for uuid in &provider_uuids {
        let key = format!("provider_{}", uuid);
        if let Some(api_key) = credentials.get(&key) {
            result.insert(uuid.clone(), api_key.clone());
            if let Ok(mut cache) = storage.cache.lock() {
                cache.insert(key, api_key.clone());
            }
        }
